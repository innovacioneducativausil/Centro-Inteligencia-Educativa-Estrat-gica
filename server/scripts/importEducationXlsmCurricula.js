...
const DOWNLOADS_DIR = 'C:/Users/ANGIE/Downloads';
...
function findSourceFiles() {
  ...
  return FILE_PATTERNS.map(pattern => {
    const matched = names.find(name => pattern.test(name));
    if (!matched) throw new Error(`No se encontró archivo con patrón ${pattern}`);
    return path.join(DOWNLOADS_DIR, matched);
  });
}
  ...
  try {
    ...
    for (const file of files) {
      ...
      try {
        ...
        await conn.commit();
        results.push({ archivo: data.archivoNombre, carrera: data.programa, ...imported });
      } catch (error) {
        await conn.rollback();
        throw error;
      }
    }
    console.log(JSON.stringify({ ok: true, version: VERSION_NAME, results }, null, 2));
  } finally {
    await conn.end();
  }
}
