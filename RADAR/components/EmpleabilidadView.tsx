...
interface Egresado {
  nro_doc: string; apellidos_nombres: string; correo_institucional: string | null;
  carrera: string; facultad: string; programa: string; anio: number;
  situacion_laboral: string | null; nivel_puesto: string | null;
  afinidad_laboral: string | null; satisfaccion_usil: string | null;
  es_emprendedor: number; salario: string | null;
  centro_laboral: string | null; rubro: string | null;
  area_trabajo: string | null; puesto_libre: string | null;
}
...
// ─── Directorio de informes (fuente: Excel histórico) ────────────────────────
interface InformeEntry { año: number; unidad: string; facultad: string; link: string; }
const INFORMES_DATA: InformeEntry[] = [
  // ── 2023 ──
  ...
  { año: 2023, unidad: 'Pregrado Ejecutivo',       facultad: '-',                     link: 'https://usiloffice365.sharepoint.com/:b:/s/ALUMNI475/IQB0eWnKw0KlQYbk_ef_b-_FAXAaudNbTTXJF9KciSYw7bQ?e=4fnRBG' },
  { año: 2023, unidad: 'Doble Grado',              facultad: '-',                     link: 'https://usiloffice365.sharepoint.com/:p:/s/ALUMNI475/IQAI2y7tXO7cQpWZghJbyswDATMuq3QLrHzko6WX68UKL80?e=Y1HIvh' },
  { año: 2023, unidad: 'Escuela de Postgrado',     facultad: '-',                     link: 'https://usiloffice365.sharepoint.com/:b:/s/ALUMNI475/IQD4wVbnTpv1TbTCFO8gUmmGAR7K42e6tqoEnxjysLQUy2A?e=ZEVQBn' },
  { año: 2023, unidad: 'Instituto de Emprendedores',facultad: '-',                    link: 'https://usiloffice365.sharepoint.com/:p:/s/ALUMNI475/IQAzmHJfCur4QYwFShn3hb3JAT7xpwL9rIIASuRQev6oJf8?e=DyiQuL' },
  // ── 2024 ──
  ...
  { año: 2024, unidad: 'Doble Grado',              facultad: '-',                     link: 'https://usiloffice365.sharepoint.com/:p:/s/ALUMNI475/IQCDvacZfL62SY3BVmUlJNTxASTGdn8tl249E3fuWF6o5Q4?e=UbCly3' },
  { año: 2024, unidad: 'Escuela de Postgrado',     facultad: '-',                     link: 'https://usiloffice365.sharepoint.com/:p:/s/ALUMNI475/IQAu2lW1o_esTIsqfEHAFRbsAWVL_Ztdo-NEF__9OUKVIgc?e=xaLbq0' },
  { año: 2024, unidad: 'Instituto de Emprendedores',facultad: '-',                    link: 'https://usiloffice365.sharepoint.com/:p:/s/ALUMNI475/IQBXxAx8KdkARKtqbrdWTYlmAcUWd4WT4o4voaJpa5zfiaY?e=x8SEAn' },
  { año: 2024, unidad: 'Global',                   facultad: '-',                     link: 'https://usiloffice365.sharepoint.com/:p:/s/ALUMNI475/IQDMfuZsF7PKT7pJBoCiJu9YAaRQQHHwkALoSLiapGwurv0?e=hbeUcY' },
  // ── 2025 ──
  ...
  { año: 2025, unidad: 'Doble Grado',              facultad: '-',                     link: 'https://usiloffice365.sharepoint.com/:p:/s/ALUMNI475/IQC2ac1YCyNASqDtr-5AdT22ASnNkALw-DstJKf3edZYU1I?e=idkbZg' },
  { año: 2025, unidad: 'Escuela de Postgrado',     facultad: '-',                     link: 'https://usiloffice365.sharepoint.com/:p:/s/ALUMNI475/IQCAY8GTfzcJRpzaLxC-z0mSAYCxud5YKjq_9XjgfiAdwhw?e=KwgIJo' },
  { año: 2025, unidad: 'Instituto de Emprendedores',facultad: '-',                    link: 'https://usiloffice365.sharepoint.com/:p:/s/ALUMNI475/IQBKQd2BGu5eQZZAtBmhtG4YAZN7wOhTPT91wiZ1YDBXtjo?e=712pup' },
];
...
const EmpleabilidadView: React.FC<EmpleabilidadViewProps> = ({ themeColors: C, userRole }) => {
  ...
  // Filtros seleccionados (dashboard general)
  ...
  // Filtros exclusivos del tab Descarga de Informes
  ...
  useEffect(() => {
    fetch('/api/empleabilidad/filtros', { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json())
      .then(d => {
        if (!d.error) {
          setFiltros(d);
  ...
  }, [selAños, selFacultad, selCarrera, selProgramas, selCiclos, token]);
  ...
  const handleNormalizarSalarios = async () => {
    if (!window.confirm('¿Normalizar rangos salariales? Esto agrupa variantes duplicadas en los rangos canónicos.')) return;
    try {
      const r = await fetch('/api/empleabilidad/admin/normalizar-salarios', {
        method: 'POST', headers: { Authorization: `Bearer ${token}` },
      });
      const d = await r.json();
      if (d.ok) {
        const msg = `✅ Normalización completada.\nInsertados: ${d.upserted}  |  Corregidos: ${d.fixed}` +
          (d.sinClasificar?.length ? `\n\nAún sin clasificar (${d.sinClasificar.length}):\n` +
            d.sinClasificar.map((x: any) => `• ${x.descripcion_original} (${x.registros} registros)`).join('\n') : '');
        window.alert(msg);
        fetchData();
      } else {
        window.alert('Error al normalizar: ' + JSON.stringify(d));
      }
  ...
  return (
    <div style={{ padding: '14px 20px', background: bg, minHeight: '100%', color: text }}>
        ...
        {canImport && (
          <div style={{ position: 'absolute', right: 0, top: 0, display: 'flex', gap: 6 }}>
            <button onClick={handleNormalizarSalarios}
              title="Normaliza rangos salariales duplicados o mal clasificados"
      ...
      {/* FILA DE FILTROS — solo visible en tabs que no son Descarga */}
      ...
      {activeTab === 'resumen' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, alignItems: 'start' }}>
            ...
            <div style={{ ...cardStyle, gridColumn: '3 / 5', padding: '14px 16px', overflow: 'visible' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    {rangos.map((r, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 7 }}>
                        <div style={{ width: 12, height: 12, borderRadius: 3, background: SALARY_COLORS[i] || '#ccc', flexShrink: 0 }} />
                        <span style={{ fontSize: 11, color: text, lineHeight: 1.3 }}>{r.rango}</span>
                        <span style={{ fontSize: 11, fontWeight: 800, color: SALARY_COLORS[i] || '#ccc', marginLeft: 'auto', flexShrink: 0 }}>{r.pct}%</span>
                      </div>
                    ))}
                  </div>
                  {/* Donut más grande con padding para las etiquetas externas */}
                  <div style={{ flexShrink: 0, padding: '4px 52px' }}>
                    <DonutChart segments={rangoSegs} size={160} stroke={32} extLabelAll />
                  </div>
                </div>
              )}
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
            ...
            <div style={{ ...cardStyle, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '12px 14px' }}>
      ...
      {activeTab === 'descarga' && (() => {
        const DL_COLOR = '#0f766e';

        // Opciones de filtro derivadas del dataset completo
        const dl_años     = ['Todos', ...[...new Set(INFORMES_DATA.map(r => String(r.año)))].sort()];
        const dl_unidades = ['Todas', ...[...new Set(INFORMES_DATA.map(r => r.unidad))].sort()];

        // Pre-filtrado por año + unidad para calcular opciones de facultad
        const preFiltered = INFORMES_DATA
          .filter(r => dlAnio === 'Todos'  || r.año    === Number(dlAnio))
          .filter(r => dlUnidad === 'Todas' || r.unidad === dlUnidad);

        const dl_facultades = ['Todas', ...[...new Set(
          preFiltered.map(r => r.facultad).filter(f => f !== '-')
        )].sort()];

        // Resultado final
        const dl_results = preFiltered
          .filter(r => dlFacultad === 'Todas' || r.facultad === dlFacultad);

        const getFacDisplay = (f: string) =>
          f === '-' ? 'General' : f === 'Todas' ? 'Todas las Facultades' : f;

        const getInformeName = (r: InformeEntry) => {
          const fac = r.facultad === '-' ? '' : r.facultad === 'Todas' ? ' Todas las Facultades' : ` ${r.facultad}`;
        ...
        const selStyle: React.CSSProperties = {
          ...
          cursor: 'pointer', fontWeight: 600, minWidth: 180,
        };
        ...
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ background: card, borderRadius: 12, border: `1px solid ${border}`,
              boxShadow: '0 2px 8px rgba(0,0,0,0.06)', padding: '16px 20px' }}>
              <div style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase',
                letterSpacing: '1px', color: DL_COLOR, marginBottom: 14 }}>
                Filtros de búsqueda
              </div>
              <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                ...
                <div>
                  <div style={{ fontSize: 9, fontWeight: 800, textTransform: 'uppercase',
                    letterSpacing: '1px', color: muted, marginBottom: 5 }}>Año</div>
                  <select value={dlAnio}
                ...
                <div>
                  <div style={{ fontSize: 9, fontWeight: 800, textTransform: 'uppercase',
                    letterSpacing: '1px', color: muted, marginBottom: 5 }}>Unidad</div>
                ...
                {dl_facultades.length > 1 && (
                  <div>
                    <div style={{ fontSize: 9, fontWeight: 800, textTransform: 'uppercase',
                      letterSpacing: '1px', color: muted, marginBottom: 5 }}>Facultad</div>
                    <select value={dlFacultad} onChange={e => setDlFacultad(e.target.value)}
                      style={selStyle}>
                      {dl_facultades.map(f => (
                        <option key={f} value={f}>{f === 'Todas' ? 'Todas las facultades' : f}</option>
                      ))}
                    </select>
                  </div>
                )}
                ...
                <button
                  onClick={() => { setDlAnio('Todos'); setDlUnidad('Todas'); setDlFacultad('Todas'); }}
                  style={{ padding: '7px 16px', borderRadius: 8, border: `1.5px solid ${border}`,
            ...
            {/* Contador */}
            <div style={{ fontSize: 12, fontWeight: 700, color: DL_COLOR }}>
              {dl_results.length} informe{dl_results.length !== 1 ? 's' : ''} encontrado{dl_results.length !== 1 ? 's' : ''}
            </div>
            ...
            {dl_results.length === 0 ? (
              ...
              <div style={{ borderRadius: 12, border: `1px solid ${border}`, overflow: 'hidden', background: card }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ background: DL_COLOR }}>
                      {['Nombre del Informe', 'Año', 'Unidad', 'Facultad', 'Acción'].map(h => (
                        <th key={h} style={{ padding: '10px 14px', textAlign: 'left',
                          fontSize: 10, fontWeight: 800, textTransform: 'uppercase',
                          letterSpacing: '0.8px', color: '#fff', whiteSpace: 'nowrap' }}>
                          {h}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {dl_results.map((r, i) => (
                      <tr key={i} style={{
                        ...
                        background: i % 2 === 0 ? card
                          : isDark ? 'rgba(255,255,255,0.025)' : 'rgba(0,0,0,0.018)',
                      }}>
                        <td style={{ padding: '10px 14px', fontSize: 12, fontWeight: 600, color: text }}>
                          {getInformeName(r)}
                        </td>
                        <td style={{ padding: '10px 14px', fontSize: 12, fontWeight: 800, color: DL_COLOR, whiteSpace: 'nowrap' }}>
                          {r.año}
                        </td>
                        <td style={{ padding: '10px 14px', fontSize: 11, color: text, whiteSpace: 'nowrap' }}>
                          {r.unidad}
                        </td>
                        <td style={{ padding: '10px 14px', fontSize: 11, color: text, whiteSpace: 'nowrap' }}>
                          {getFacDisplay(r.facultad)}
                        </td>
                        <td style={{ padding: '10px 14px' }}>
                          <a href={r.link.trim()} target="_blank" rel="noopener noreferrer"
                            style={{ display: 'inline-flex', alignItems: 'center', gap: 5,
                              padding: '6px 14px', borderRadius: 7, background: DL_COLOR,
              ...
              <div style={{ flex: 1 }}>
                {items.map((it, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 10, color: text, flex: 1, lineHeight: 1.3 }}>{it.label}</span>
                ...
                {S.satisfaccion.length === 0 ? <EmptyState /> : (
                  <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                    <div style={{ flexShrink: 0, padding: '0 24px' }}>
                      <DonutChart
                        segments={S.satisfaccion.map((it, i) => ({ pct: it.pct, color: getSatisfColor(it.label) }))}
